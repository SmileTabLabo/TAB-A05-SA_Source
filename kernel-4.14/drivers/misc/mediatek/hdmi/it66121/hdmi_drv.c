...
static int it66121_irq = -1;
static atomic_t it66121_irq_disabled = ATOMIC_INIT(0);
static struct workqueue_struct *it66121_wq = NULL;
static struct work_struct it66121_work;
...
void HDMI_reset(void)
{
	...
	reset_pin = of_get_named_gpio(dn, "hdmi_reset_gpios", 0);
...
#endif
	if (atomic_dec_and_test(&it66121_irq_disabled)) {
//		pr_info("enable irq @ %s\n", __FUNCTION__);
		enable_irq(it66121_irq);
	}
...
static irqreturn_t it66121_irq_handler(int irq, void *data)
{
//	pr_info("it66121 irq\n");

	disable_irq_nosync(irq);
	atomic_set(&it66121_irq_disabled, 1);
	queue_work(it66121_wq, &it66121_work);

	return IRQ_HANDLED;
}

static void it66121_irq_work(struct work_struct *work)
{
//	pr_info("it66121 irq work (%d,%d)\n",
//			hdmi_powerenable, atomic_read(&it66121_irq_disabled));
				...
				enum HDMI_VIDEO_INPUT_FORMAT vin, int vout)
{
	...
	else if (vformat == HDMI_VIDEO_1920x1080p_24Hz)
		it66121_video_type = HDMI_1080p24;
	else if (vformat == HDMI_VIDEO_1920x1080p_25Hz)
		it66121_video_type = HDMI_1080p25;
...
static void it66121_get_params(struct HDMI_PARAMS *params)
{
	...
	case HDMI_VIDEO_1920x1080p_24Hz:
		...
		params->width = 1920;
		params->height = 1080;
		params->input_clock = HDMI_VIDEO_1920x1080p_24Hz;
		params->init_config.vformat = HDMI_VIDEO_1920x1080p_24Hz;
		break;
	case HDMI_VIDEO_1920x1080p_25Hz:
		...
		params->width = 1920;
		params->height = 1080;
		params->input_clock = HDMI_VIDEO_1920x1080p_25Hz;
		params->init_config.vformat = HDMI_VIDEO_1920x1080p_25Hz;
		break;
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (strcmp(client->name, "it66121-i2c") == 0) {
		...
		if (it66121_i2c_client != NULL) {
			...
			it66121_wq =
				create_singlethread_workqueue("it66121_wq");
			INIT_WORK(&it66121_work, it66121_irq_work);

#ifdef CONFIG_OF
			irq_gpio = of_get_named_gpio(
				client->dev.of_node, "irq-gpio", 0);
			pr_notice("[%s] of_get_named_gpio(): %d\n",
					  __func__, irq_gpio);

			if (irq_gpio >= 0) {
				gpio_request(irq_gpio, NULL);
				gpio_direction_input(irq_gpio);
				client->irq = gpio_to_irq(irq_gpio);
				it66121_irq = client->irq;
				atomic_set(&it66121_irq_disabled, 0);
				request_threaded_irq(
					client->irq, NULL, it66121_irq_handler,
...
int ite66121_pmic_power_on(void)
{
	...
	if (hdmi_use_pmic_supply) {
		...
		ret = regulator_enable(hdmi_vbst_5v);
		if (ret != 0)
...
int ite66121_pmic_power_off(void)
{
	...
	if (hdmi_use_pmic_supply) {
		...
		ret = regulator_disable(hdmi_vbst_5v);
		if (ret != 0)
			IT66121_LOG("hdmi regolator error\n");
	}
	IT66121_LOG("%s\n", __func__);
...
int hdmi_internal_probe(struct platform_device *pdev)
{
	...
	if (hdmi_use_pmic_supply) {
		...
		hdmi_vbst_5v = devm_regulator_get(&pdev->dev, "vbst_5v");
		...
		if (IS_ERR(hdmi_vbst_5v))
			IT66121_LOG("hdmi hdmi_vbst_5v error\n");
	} else {
