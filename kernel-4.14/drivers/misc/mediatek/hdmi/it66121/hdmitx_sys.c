...
#else
	if (tmp_HPDChange && (tmp_HPD == HPDStatus)) {
		pr_warning("tmp_HPDChange=%d, tmp_HPD==HPD_Status(%d)\n",
			tmp_HPDChange, tmp_HPD);
		if (tmp_HPD) {
...
/* HDMITX_DisableVideoOutput(); */
		...
		if (HPDStatus) {
...
#endif
			{
				...
				hdmitx_hdcp_auth_start(bHDMIMode);
#endif /* SUPPORT_HDCP_STS */
				it66121_bChangeMode = FALSE;
			}
			/* /hh test */
...
int vesa_standerd_timing_cmp(
	unsigned char *EDID_Buf, unsigned char b1, unsigned char b2)
{
	int i;

	for (i=0; i<16; i+=2) {
		if ((EDID_Buf[0x26 + i] == b1) && (EDID_Buf[0x27 + i] == b2))
			return 1;
	}
	return 0;
}
...
unsigned char ParseEDID(void)
{
	...
	if (BlockCount == 0) {
		...
		if (vesa_standerd_timing_cmp(EDID_Buf, 0x81, 0xc0))
...
{
//	if(sink_support_resolution == 0)
//		sink_support_resolution |= SINK_480P | SINK_720P60 | SINK_1080P30 | SINK_1080P25 | SINK_1080P24;
