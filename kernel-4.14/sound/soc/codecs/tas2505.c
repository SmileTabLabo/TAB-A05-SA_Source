...
#define TAS2505_X2_SUPPORT

#ifdef TAS2505_X2_SUPPORT
static struct i2c_client *tas2505_lch = NULL;
static struct i2c_client *tas2505_rch = NULL;
#endif /* TAS2505_X2_SUPPORT */
...
static struct tas2505_priv * tas2505_get_drvdata(struct device *dev)
{
	return (struct tas2505_priv *)dev_get_drvdata(dev);
}
...
static void tas2505_init_regs(struct tas2505_priv *tas2505)
{
	...
	regmap_write(rm, TAS2505_PLLJ, 0x04);
	regmap_write(rm, TAS2505_PLLDMSB, 0x00);
	regmap_write(rm, TAS2505_PLLDLSB, 0x00);
	usleep_range(15, 20);
	regmap_write(rm, TAS2505_NDAC, 0x84);
	...
	regmap_write(rm, TAS2505_DACSETUP1, tas2505->channel_setup[0]);
	regmap_write(rm, TAS2505_DACVOL, 0x00);
	regmap_write(rm, TAS2505_DACSETUP2, tas2505->channel_setup[1]);
}
	...
	const struct i2c_device_id *id)
{
	...
	tas2505->power_gpio = -1;
	tas2505->reset_gpio = -1;
	...
	if ((ret > 0) && gpio_is_valid(ret)) {
		tas2505->power_gpio = ret;
		devm_gpio_request(&i2c->dev, tas2505->power_gpio, "power");
		gpio_direction_output(tas2505->power_gpio, 1);
	...
	if ((ret > 0) && gpio_is_valid(ret)) {
		tas2505->reset_gpio = ret;
		devm_gpio_request(&i2c->dev, tas2505->reset_gpio, "reset");
		gpio_direction_output(tas2505->reset_gpio, 1);
		usleep_range(1000, 2000);
	}

	ret = regmap_read(tas2505->regmap, TAS2505_PAGECTL, &regval);
	if (ret != 0) {
		dev_err(&i2c->dev, "failed to read Page Select register (%d)\n",
			ret);
		return ret;
	}

	/* DAC Channel Setup Register 1/2 value */
	ret = of_property_read_u8_array(
		np, "channel-setup", tas2505->channel_setup, 2);
	if (ret == 0) {
#ifdef TAS2505_X2_SUPPORT
		dev_info(&i2c->dev, "channel-setup=<0x%02x 0x%02x>\n",
			tas2505->channel_setup[0], tas2505->channel_setup[1]);
		switch (tas2505->channel_setup[0] & 0x30)
		{
		case 0x10:	/* L-ch */
			tas2505_lch = i2c;
			dev_info(&i2c->dev, "L-ch=%p\n", tas2505_lch);
			break;
		case 0x20:	/* R-ch */
			tas2505_rch = i2c;
			dev_info(&i2c->dev, "R-ch=%p\n", tas2505_lch);
			break;
		default:
			break;
		}
#endif /* TAS2505_I2C_SUPPORT */
	} else {
		dev_err(&i2c->dev, "failed to get 'channel-setup' value\n");
		tas2505->channel_setup[0] = 0x14;
		tas2505->channel_setup[1] = 0x0c;
	}

	tas2505_init_regs(tas2505);
...
#ifndef TAS2505_X2_SUPPORT
#ifdef CONFIG_PM_SLEEP
static int tas2505_i2c_suspend(struct device *dev)
{
	struct tas2505_priv *tas2505 = dev_get_drvdata(dev);

	if (gpio_is_valid(tas2505->reset_gpio)) {
		gpio_set_value(tas2505->reset_gpio, 0);
	}

	return 0;
}

static int tas2505_i2c_resume(struct device *dev)
{
	struct tas2505_priv *tas2505 = dev_get_drvdata(dev);

	if (gpio_is_valid(tas2505->reset_gpio)) {
		gpio_set_value(tas2505->reset_gpio, 1);
		usleep_range(1000, 2000);
	}
	tas2505_init_regs(tas2505);
...
static SIMPLE_DEV_PM_OPS(
	tas2505_pm_ops, tas2505_i2c_suspend, tas2505_i2c_resume);
...
#else /* TAS2505_X2_SUPPORT */

/*****************************************************************************/

static int tas2505_power_gpio = 0;
static int tas2505_reset_gpio = 0;

static int tas2505_x2_regmap_write(unsigned int reg, unsigned int val)
{
	int ret;

	/* L-ch */
	ret = regmap_write(
		tas2505_get_drvdata(&tas2505_lch->dev)->regmap, reg, val);
	if (ret != 0) {
		return ret;
	}
	...
	/* R-ch */
	ret = regmap_write(
		tas2505_get_drvdata(&tas2505_rch->dev)->regmap, reg, val);
	if (ret != 0) {
		return ret;
	}
...
static int tas2505_x2_codec_probe(struct snd_soc_codec *codec)
{
	dev_dbg(codec->dev, "%s\n", __func__);
	return 0;
}

static int tas2505_x2_codec_remove(struct snd_soc_codec *codec)
{
	dev_dbg(codec->dev, "%s\n", __func__);
	return 0;
}

static int tas2505_x2_codec_write(
	struct snd_soc_codec *codec, unsigned int reg, unsigned int val)
{
	dev_dbg(
		codec->dev, "%s: reg=0x%02x, val=0x%02x\n", __func__,
		reg, val);
	return tas2505_x2_regmap_write(reg, val);
}

static struct snd_soc_codec_driver tas2505_x2_codec_driver = {
	.probe = tas2505_x2_codec_probe,
	.remove = tas2505_x2_codec_remove,
	.write = tas2505_x2_codec_write,
};

/*****************************************************************************/

static int tas2505_x2_dai_digital_mute(
	struct snd_soc_dai *dai, int mute)
{
	dev_dbg(dai->codec->dev, "%s: mute=%d\n", __func__, mute);

	if (mute) {
		snd_soc_write(dai->codec, TAS2505_DACSETUP2, 0x0c);
	} else {
		snd_soc_write(dai->codec, TAS2505_DACSETUP2, 0x04);
	}
	msleep(10);
...
static const struct snd_soc_dai_ops tas2505_x2_dai_ops = {
	.digital_mute = tas2505_x2_dai_digital_mute,
};

static struct snd_soc_dai_driver tas2505_x2_dai_driver[] = {
	{
		.name = "tas2505_x2-hifi",
		.playback = {
			.stream_name	= "Playback",
			.channels_min	= 1,
			.channels_max	= 2,
...
static int tas2505_x2_platform_probe(struct platform_device *pdev)
{
	int ret;
	...
	ret = of_get_named_gpio(np, "gpio-power", 0);
	if ((ret > 0) && gpio_is_valid(ret)) {
		tas2505_power_gpio = ret;
		devm_gpio_request(&pdev->dev, tas2505_power_gpio, "power");
		gpio_direction_output(tas2505_power_gpio, 1);
		usleep_range(1000, 2000);
	}
	...
	ret = of_get_named_gpio(np, "gpio-reset", 0);
	if ((ret > 0) && gpio_is_valid(ret)) {
		tas2505_reset_gpio = ret;
		devm_gpio_request(&pdev->dev, tas2505_reset_gpio, "reset");
		gpio_direction_output(tas2505_reset_gpio, 1);
		usleep_range(1000, 2000);
	}

	/* register I2C devices */
	ret = i2c_add_driver(&tas2505_i2c_driver);
	if (ret != 0) {
		dev_err(&pdev->dev, "%s: failed to register i2c driver (%d)\n",
			__func__, ret);
		return ret;
	}
	dev_info(&pdev->dev, "i2c_client: L-ch=%p, R-ch=%p\n",
		tas2505_lch, tas2505_rch);

	dev_info(&pdev->dev, "%s done\n", __func__);

	return snd_soc_register_codec(
		&pdev->dev, &tas2505_x2_codec_driver,
		tas2505_x2_dai_driver, ARRAY_SIZE(tas2505_x2_dai_driver));
}

static int tas2505_x2_platform_remove(struct platform_device *pdev)
{
	i2c_del_driver(&tas2505_i2c_driver);
	snd_soc_unregister_codec(&pdev->dev);
	return 0;
}

#ifdef CONFIG_PM_SLEEP
static int tas2505_x2_platform_suspend(struct device *dev)
{
	dev_dbg(dev, "%s\n", __func__);

	if (gpio_is_valid(tas2505_reset_gpio)) {
		gpio_set_value(tas2505_reset_gpio, 0);
		usleep_range(1000, 2000);
	}
...
static int tas2505_x2_platform_resume(struct device *dev)
{
	dev_dbg(dev, "%s\n", __func__);

	if (gpio_is_valid(tas2505_reset_gpio)) {
		gpio_set_value(tas2505_reset_gpio, 1);
		usleep_range(1000, 2000);
	}

	tas2505_init_regs(tas2505_get_drvdata(&tas2505_lch->dev));
	tas2505_init_regs(tas2505_get_drvdata(&tas2505_rch->dev));
...
static SIMPLE_DEV_PM_OPS(
	tas2505_x2_pm_ops,
	tas2505_x2_platform_suspend, tas2505_x2_platform_resume);
#endif /* CONFIG_PM_SLEEP */

static const struct of_device_id tas2505_x2_of_match[] = {
	{ .compatible = "ti,tas2505_x2" },
	{},
};
MODULE_DEVICE_TABLE(of, tas2505_x2_of_match);

static struct platform_driver tas2505_x2_platform_driver = {
	.driver = {
		.name = "tas2505_x2-codec",
		.of_match_table = tas2505_x2_of_match,
#ifdef CONFIG_PM_SLEEP
		.pm = &tas2505_x2_pm_ops,
#endif
	},
	.probe = tas2505_x2_platform_probe,
	.remove = tas2505_x2_platform_remove,
};

module_platform_driver(tas2505_x2_platform_driver);
